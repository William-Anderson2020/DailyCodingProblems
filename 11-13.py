...
def newUnit(name,ref,value):
    units[name]=units[ref]*value
    
newUnit("foot","inch",12)
newUnit("yard","foot",3)
newUnit("chain","yard",22)

print(units)

def convert(init,req,val):
    result = (units[init]*val)/units[req]
    print("{} {} => {} {}".format(val,init,result,req))
    return result
