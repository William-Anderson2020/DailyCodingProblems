...
def minDist(people,towers):
    minDist = 0
    for person in people:
        minDistPerson = 0
        for tower in towers:
            val = abs(person-tower)
            if val < minDistPerson or minDistPerson == 0:
                minDistPerson = val
        if minDistPerson > minDist or minDist == 0:
            minDist = minDistPerson
    return minDist

print(minDist([1,5,11,20],[4,8,15]))
